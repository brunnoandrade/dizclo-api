...
export class PartnerTestmonialService {
    ...
    async update(username: string, id: string, data: PartnerTestmonial): Promise<Partner> {
        return await this.model.findOneAndUpdate({ username, 'testmonials._id': id }, { // active: true
            $set: {
                'testmonials.$': data,
            }
        });
